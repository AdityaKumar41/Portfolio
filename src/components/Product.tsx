  ...
  return (
    <div className="py-10">
      ...
      >
        ...
        />
        <div className="absolute bottom-0 bg-white dark:bg-black h-40 w-full [mask-image:linear-gradient(to_bottom,transparent,white)]" />
      </motion.div>
      ...
      <div className="flex lg:flex-row justify-between items-center flex-col mt-20">
        ...
        <div className="flex md:mb-1 mt-2 md:mt-0 flex-wrap">
          {product.stack?.map((stack: string) => (
            <span
              key={stack}
              className="text-xs  md:text-xs lg:text-xs bg-[#212121] text-white dark:bg-[#212121] px-2 py-1 my-1 mx-1 rounded-sm text-secondary"
            >
              {stack}
            </span>
          ))}
      ...
      <div>
        <Paragraph className="max-w-xl mt-4 text-neutral-900 dark:text-neutral-200">{product.description}</Paragraph>
      </div>
      <div className="prose prose-sm md:prose-base max-w-none text-neutral-700 dark:text-neutral-400">
        {product?.content}
      </div>
